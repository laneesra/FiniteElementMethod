...
class Detail:
    ...
    def __init__(self):  # customized for detail
        ...
        self.borders = [(self.points[4], self.points[5]), (self.points[5], self.points[6]), (self.points[6], self.points[2]),
                        (self.points[2], self.points[3]), (self.points[3], self.points[1]), (self.points[1], self.points[0]),
...
class Element:
    def __init__(self, index, s1, s2, s3):
        ...
        self.borders = {'12': {'type': '', 'val': 0, 'length': L12, 'vector': np.array([1, 1, 0])},
                        '23': {'type': '', 'val': 0, 'length': L23, 'vector': np.array([0, 1, 1])},
                        '31': {'type': '', 'val': 0, 'length': L31, 'vector': np.array([1, 0, 1])} }
    ...
    def form_elem_matrix(self, Kxx, Kyy):
        ...
        if self.borders['12']['type'] == 'convective_heat_transfer':
            k += self.borders['12']['val'] * self.borders['12']['length'] / 6 * np.array([[2, 1, 0], [1, 2, 0], [0, 0, 0]])
        if self.borders['23']['type'] == 'convective_heat_transfer':
            k += self.borders['23']['val'] * self.borders['23']['length'] / 6 * np.array([[0, 0, 0], [0, 2, 1], [0, 1, 2]])
        if self.borders['31']['type'] == 'convective_heat_transfer':
            k += self.borders['31']['val'] * self.borders['31']['length'] / 6 * np.array([[2, 0, 1], [0, 0, 0], [1, 0, 2]])
        return k
    ...
    def form_vector_of_external_influences(self, point_sources):
        ...
        for k in self.borders:
            if self.borders[k]['val'] != 0:
                if self.borders[k]['type'] == 'heat_flow':
                    f += self.borders[k]['val'] / 2 * (self.borders[k]['length'] * self.borders[k]['vector'])
                elif self.borders[k]['type'] == 'convective_heat_transfer':
...
class FEM:
        ...
        for elem in self.elements:
            for p in self.detail.source_points:
                if abs(p.x - elem.s1.x) < 0.0001 and abs(p.y - elem.s1.y) < 0.0001:
                    ...
                    elem.ps = False
            elem.grad = 1 / (2*elem.A) * np.dot(np.array([[elem.b[0], elem.b[1], elem.b[2]], [elem.c[0], elem.c[1], elem.c[2]]]),
    ...
    def create_vtu(self, file):
        ...
        output += '\n\t\t\t<DataArray type="UInt32" Name="connectivity" format="ascii">{}</DataArray>'.format(connectivity)
